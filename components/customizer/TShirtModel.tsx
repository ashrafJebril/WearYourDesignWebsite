...
function useTShirtLeftSleeveDecalTransform(
  ...
  return useMemo(() => {
    ...
    // Left sleeve positioning - on the left side of the shirt
    const posX = box.min.x + size.x * 0.1;
    const posY = center.y + decalPosition.y * size.y * 0.003 + size.y * 0.25;
    const posZ = center.z + decalPosition.x * size.z * 0.003;

    const decalSize = decalScale * size.y * 0.12;
    // Use a very small depth to prevent decal from projecting through the thin T-shirt mesh
    const decalDepth = decalSize * 0.05;
...
function useTShirtRightSleeveDecalTransform(
  ...
  return useMemo(() => {
    ...
    // Right sleeve positioning - on the right side of the shirt
    const posX = box.max.x - size.x * 0.1;
    const posY = center.y + decalPosition.y * size.y * 0.003 + size.y * 0.25;
    const posZ = center.z + decalPosition.x * size.z * 0.003;
...
function useTShirtFrontDecalTransform(
  ...
  return useMemo(() => {
    ...
    const posZ = box.max.z + zOffset + 0.001;

    const decalSize = decalScale * size.y * 0.4;
...
function useTShirtBackDecalTransformNew(
  ...
  return useMemo(() => {
    ...
    const posX = center.x + decalPosition.x * 0.001;
    const posY = center.y + decalPosition.y * 0.001 + size.y * 0.05;
