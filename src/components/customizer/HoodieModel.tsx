...
function useBackDecalTransform(
  ...
  return useMemo(() => {
    ...
    // Position on the back - mirror of front positioning
    const posX = center.x + decalPosition.x - 16;
    const posY = center.y + decalPosition.y + 70;
    // Back side: use min Z and push slightly outward (negative direction)
    const posZ = box.min.z + 100 + zOffset;

    const decalSize = decalScale * Math.min(size.x, size.y) * 0.3;

    console.log("Back decal transform:", { posX, posY, posZ, decalSize, boxMin: box.min.z, boxMax: box.max.z });
...
export function HoodieModel() {
  ...
  useEffect(() => {
    ...
    loader.load(
      backImage,
      (texture) => {
        console.log("Back image texture loaded successfully");
